...
public:
    LTMatrix(int n){
        this->n = n;
        A = new int [n * (n + 1)/2]; // We need to initialize (n(n+1)/2) non zero elements
    }
    ~LTMatrix(){ delete[] A; }
...
void LTMatrix::setRowMajor(int i, int j, int x) {
    if (i >= j){
        int index = ((i * (i - 1))/2) + j - 1; //Index in RowMajor is {[i(i-1)/2] + (j-1)}
        A[index] = x;
    }
}
...
void LTMatrix::setColMajor(int i, int j, int x) {
    if (i >= j){
        int index = (n * (j-1) - (((j-2) * (j-1))/2)) + (i-j);//Index in Col Major is n(j-1)-((j-2)*(j-1)/2)+(i-j)
        A[index] = x;
    }
}
...
int main() {
    ...
    cout<<"Row major output is as below"<<endl;
    rm.Display(true);
    cout << endl;
    ...
    cout<<"Column major output is as below"<<endl;
    cm.Display(false);
